...
{
    /* one-vs-rest AUC (binary) – trapezoidal rule ----------------------- */
...
int main(int argc,char* argv[])
{
    ...
    /* ----------- dataset / loader ------------------------------------- */
    auto test_ds = ADNIDataset(cfg,"test")
                   .map(torch::data::transforms::Stack<>());
    auto test_ld = torch::data::make_data_loader<torch::data::samplers::SequentialSampler>(
        std::move(test_ds),
        torch::data::DataLoaderOptions().batch_size(cfg.test_batch_sz)
                                        .workers(cfg.workers));
    ...
    std::vector<std::vector<double>> all_prob(C);   // per-classe scores
    ...
    for (auto& b : *test_ld){
        ...
        for (int i=0; i<y.size(0); ++i){
            ...
            for (int c=0;c<C;++c)
                all_prob[c].push_back(prob_cpu[i][c].item<double>());
        }
    }
    ...
    const double macro_auc =
        std::accumulate(class_auc.begin(),class_auc.end(),0.0)/C;
    ...
    fout<<header<<"\ttrue_label\tpredicted_label\n";

    std::map<int,std::string> map_classes =
        (C==3)?
        std::map<int,std::string>{{0,"CN"},{1,"LMCI"},{2,"AD"}} :
