...
struct Simple3DCNNImpl : torch::nn::Module {
    ...
    Simple3DCNNImpl(int in_channels = 1, int num_classes = 3) {
        // conv blocks
        conv1   = register_module("conv1",   torch::nn::Conv3d(in_channels, 32, 3, /*stride=*/1, /*padding=*/1));
        bn1     = register_module("bn1",     torch::nn::BatchNorm3d(32));
        conv2   = register_module("conv2",   torch::nn::Conv3d(32, 64, 3, 1, 1));
        bn2     = register_module("bn2",     torch::nn::BatchNorm3d(64));
        conv3   = register_module("conv3",   torch::nn::Conv3d(64,128, 3, 1, 1));
